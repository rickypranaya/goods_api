...
goodsdb.search_get = (params)=>{
    ...
    return new Promise((resolve,reject)=>{
        pool.query('SELECT * FROM transaction WHERE (customer_name LIKE N? OR no LIKE N? OR catatan LIKE N?) AND marketplace = ? ORDER BY created_at DESC' ,[keyword, params.marketplace ],(err,results)=>{
            if (err){
                return reject (err);
            } 
            return resolve (results);
