...
goodsdb.modal_get = (params)=>{
    return new Promise((resolve,reject)=>{
        pool.query('SELECT modal, marketplace FROM transaction where  created_at >= ? and  created_at <= ? GROUP BY marketplace', [ params.start_month, params.end_month],(err,results)=>{
            if (err){
                return reject (err);
            } 
            return resolve (results);
